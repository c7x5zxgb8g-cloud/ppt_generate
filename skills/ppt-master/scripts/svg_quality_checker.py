...
class SVGQualityChecker:
    ...
    def check_file(self, svg_file: str, expected_format: str = None) -> Dict:
        ...
        try:
            ...
            if self._check_xml_well_formed(content, result):
                ...
                # 9. Check courseware layer contract when courseware mode is enabled
                self._check_courseware_layers(content, svg_path, result)
    ...
    def _check_courseware_layers(self, content: str, svg_path: Path, result: Dict):
        """Validate the top-level layer model for courseware decks.

        Triggered only when spec_lock.md contains:

            ## courseware
            - mode: enabled

        This keeps the generic presentation workflow backward compatible while
        making courseware mode enforce staged teaching layers.
        """
        lock = self._get_spec_lock(svg_path)
        if lock is None:
            return
        courseware = lock.get('courseware', {})
        if str(courseware.get('mode', '')).strip().lower() != 'enabled':
            return
        ...
        top_level_elements = [
            ...
            if _local(child.tag) not in {'defs', 'title', 'desc', 'metadata'}
        ]
        layer_ids = []
        raw_top_level = []
        for child in top_level_elements:
            tag = _local(child.tag)
            child_id = child.attrib.get('id', '')
            if tag == 'g' and child_id.startswith('layer-'):
                layer_ids.append(child_id)
            else:
                raw_top_level.append(tag if not child_id else f"{tag}#{child_id}")
        ...
        required = [
            ...
            for item in layers.get(
                'required',
                'layer-background, layer-content, layer-chrome'
            ).split(',')
            if item.strip()
        ]
        missing = [layer for layer in required if layer not in layer_ids]
        if missing:
            result['errors'].append(
                f"Courseware layer model missing required layer(s): {', '.join(missing)}"
            )

        raw_policy = layers.get('raw_top_level_elements', 'forbidden').strip().lower()
        if raw_policy == 'forbidden' and raw_top_level:
            sample = ', '.join(raw_top_level[:5])
            result['errors'].append(
                f"Courseware layer model forbids raw top-level SVG elements; found: {sample}. "
                "Wrap teaching units inside layer-* groups."
            )

        page_key = None
        m = re.match(r'(\d{2})[_-]', svg_path.name)
        if m:
            page_key = f"P{int(m.group(1)):02d}"
        page_type = None
        if page_key:
            raw_page_type = lock.get('page_types', {}).get(page_key, '')
            if raw_page_type:
                page_type = raw_page_type.split('|', 1)[0].strip()

        feedback_required = {
            item.strip()
            for item in layers.get('feedback_layer_required_for', 'quiz').split(',')
            if item.strip()
        }
        if page_type in feedback_required and 'layer-feedback' not in layer_ids:
            result['errors'].append(
                f"Courseware page type '{page_type}' requires layer-feedback for staged reveal"
            )
