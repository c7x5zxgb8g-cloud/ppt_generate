...
description: >
  ...
  multi-role collaboration. Also supports a vertical courseware mode for teen/K-12
  education decks with knowledge-point planning and layered teaching slides. Use when
  user asks to "create PPT", "make presentation", "生成PPT", "做PPT", "制作演示文稿",
  "生成课件", "做课件", "教学PPT", or mentions "ppt-master".
---
...
>
> **Courseware mode**: for youth education / K-12 / 教培课件, the workflow becomes a teaching-design pipeline: source material → knowledge-point extraction → lesson/page-type planning → layered SVG courseware → editable PPTX.
...
| `${SKILL_DIR}/scripts/extract_knowledge_points.py` | Courseware mode: extract knowledge points and draft page-type plan |
...
## Courseware Mode Resources

| Resource | Path | Purpose |
|----------|------|---------|
| Page type system | `references/courseware-types.md` | Objectives / concept / example / quiz / comparison / diagram / summary rules |
| Layer model | `references/layer-model.md` | Required top-level SVG layer groups for staged teaching and editing |
| Courseware lock extension | `references/spec-lock-courseware.md` | Extra `spec_lock.md` sections for courseware metadata |
| Layout templates | `references/layout-templates/` | Five MVP courseware layout references |
...
### Step 1.5: Courseware Knowledge Extraction (Conditional)

🚧 **GATE**: Step 1 complete; source content is ready.

**Trigger**: user asks for youth education / K-12 / 教培 / 课件 / 教学PPT, or the source is clearly a lesson plan, textbook, curriculum standard, exercise sheet, or classroom script.

When triggered, run the courseware extractor against the normalized Markdown:

```bash
python3 ${SKILL_DIR}/scripts/extract_knowledge_points.py <source.md> \
  -o <project_or_source_dir>/course_outline.json
```

If the project has not been initialized yet, write `course_outline.json` next to the source Markdown first; after Step 2 import it into `sources/` with the other source artifacts.

The extractor is an MVP structural draft, not an authority. Strategist MUST review and refine the output using `references/courseware-types.md`.

**✅ Checkpoint — Courseware outline draft ready, proceed to Step 2.**
...
If Courseware Mode is triggered, also read:
```
Read references/courseware-types.md
Read references/layer-model.md
Read references/spec-lock-courseware.md
Read references/layout-templates/objectives-layout.md
Read references/layout-templates/concept-layout.md
Read references/layout-templates/example-layout.md
Read references/layout-templates/quiz-layout.md
Read references/layout-templates/summary-layout.md
```
...
**Courseware Mode additional output requirements**:
- `design_spec.md §IX` MUST include `courseware_type`, `knowledge_point_id`, `difficulty`, `bloom_level`, `estimated_minutes`, `teaching_move`, and `layer_plan` for every page.
- `spec_lock.md` MUST include courseware extension sections from `references/spec-lock-courseware.md`: `courseware`, `knowledge_points`, `page_types`, and `layers`.
- Every quiz page MUST put answer/explanation in the planned `layer-feedback` layer.
...
**Courseware Layer Confirmation (Mandatory when enabled)**: if `spec_lock.md` has `courseware.mode: enabled`, output the required layer set and reveal order before the first SVG. Every page SVG MUST be authored as top-level `layer-*` groups, not flat top-level elements. See `references/layer-model.md`.
...
- In Courseware Mode, missing required `layer-*` groups or raw top-level SVG elements are `error`s and MUST be fixed before proceeding.
