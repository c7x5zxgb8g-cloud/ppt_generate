...
**Courseware layer gate**: when `spec_lock.md` contains `courseware.mode: enabled`, read
`references/layer-model.md` before generating the first page. Every SVG page MUST use
top-level `layer-*` groups instead of flat top-level shapes. Required layers are read from
`spec_lock.md ## layers` (default: `layer-background`, `layer-content`, `layer-chrome`).
Quiz and worked-example answers belong in `layer-feedback`; hints belong in
`layer-guidance`; student action areas belong in `layer-interaction`.
