...
- **Courseware layers**: When `spec_lock.md` declares `courseware.mode: enabled`, direct children of `<svg>` must be semantic `layer-*` groups (`layer-background`, `layer-content`, `layer-chrome`, plus optional `layer-context`, `layer-interaction`, `layer-guidance`, `layer-feedback`). See `references/layer-model.md`.
