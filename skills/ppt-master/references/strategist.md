...
When Courseware Mode is triggered (K-12 / youth education / 教培 / 课件 / 教学PPT), act as a **senior instructional designer + subject-matter teaching expert**. The goal is not only attractive slides; it is student understanding, practice, retention, and classroom usability.
...
🚧 **Courseware gate — if triggered**: also read `references/courseware-types.md`, `references/layer-model.md`, `references/spec-lock-courseware.md`, and the relevant files under `references/layout-templates/` before drafting confirmations. If `sources/course_outline.json` exists, read it and use it as a draft knowledge graph, then correct it from the source material.
...
| **D) Courseware** | Learning effectiveness first | Teen/K-12 students and teachers | "Teach, practice, reveal, check" |

**Style selection decision tree**:

```
Content characteristics?
  ├── Lesson / textbook / exercises / classroom script ──→ D) Courseware
...
Audience?
  ├── Teen/K-12 students / teachers / tutoring orgs → D) Courseware
...
### D) Courseware — Executor_General + Courseware Layer Model

- **Capabilities**: teaching objectives, concept explanation, worked examples, in-class practice, feedback reveal, summary maps.
- **Scenarios**: K-12 lessons, youth training, tutoring decks, textbook-to-courseware conversion.
- **Avoid**: flat content-showcase pages, all answers visible on quiz slides, too many new terms on one page.
- **Required references**: `courseware-types.md`, `layer-model.md`, and layout templates.
- **Default page type sequence**: objectives → concept → example → quiz → concept/diagram → quiz → summary.
- **Layer rule**: every SVG must use top-level `layer-*` groups; `layer-feedback` is mandatory for quiz pages and recommended for examples.

## 2.1 Courseware Planning Requirements

When Courseware Mode is active, `design_spec.md §IX Content Outline` must become a lesson plan, not a simple slide list. For every page include:

```markdown
#### Slide 03 - 等式的性质
- courseware_type: concept
- knowledge_point_id: KP02
- difficulty: medium
- bloom_level: understand
- estimated_minutes: 6
- teaching_move: Use a balance-scale analogy, then ask students to predict both-side operations.
- layer_plan: layer-background / layer-context / layer-content / layer-interaction / layer-guidance / layer-chrome
```

Also add a knowledge point table near the start of §IX or in §VII when no visualization table is needed:

| ID | Knowledge Point | Difficulty | Bloom | Minutes | Prerequisites |
| -- | --------------- | ---------- | ----- | ------- | ------------- |
| KP01 | 方程的定义 | easy | remember | 4 | — |

`spec_lock.md` must include:
...
The layer plan must follow `references/layer-model.md`. Do not let Executor infer layers from a flat outline.
